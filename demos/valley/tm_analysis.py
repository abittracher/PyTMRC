...
potential = potential.Potential(ystretch=20)
...
def computeKernelRC():
    ...
    kerTM = tm.KernelBurstTransitionManifold(system, kernel, xtest, .1, 0.01, 100, 1)
    kerTM.computeRC()
    return kerTM
...
def visualizeTrajectory():
    ...
    x0 = np.array([[0, 0]])
    x = system.computeTrajectory(.1,0.00001,x0)
